...
int main(int argc, char *argv[]) {
    ...
    rc = ecx_FPRD(&context.port, slave, 0x0000, sizeof(type_val),
                  &type_val, EC_TIMEOUTRET);
    printf("  Read Type   (0x0000): 0x%04X (%s)\n", type_val,
           rc > 0 ? "OK" : "FAILED");

    rc = ecx_FPRD(&context.port, slave, 0x0001, sizeof(revision_val),
                  &revision_val, EC_TIMEOUTRET);
    ...
    rc = ecx_FPWR(&context.port, slave, 0x0F00, sizeof(gpio_wr),
                  &gpio_wr, EC_TIMEOUTRET);
    printf("  Write GPIO  (0x0F00) = 0x%02X: %s\n", gpio_wr,
           rc > 0 ? "OK" : "FAILED");

    usleep(1000);

    rc = ecx_FPRD(&context.port, slave, 0x0F00, sizeof(gpio_rd),
                  &gpio_rd, EC_TIMEOUTRET);
    ...
    if (st == EC_STATE_PRE_OP) {
        ...
        printf("  PREOP:    FAILED (state=0x%04X, ALstatuscode=0x%04X)\n",
               st, context.slavelist[0].ALstatuscode);
    }
