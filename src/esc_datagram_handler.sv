    ...
    if (rx_valid) begin
      ...
      if (is_ethercat) begin
        if (byte_idx == cmd_idx) begin
          ...
          if (is_auto_inc_cmd)
            tx_byte_comb = rx_data - 8'h01;
        end else if (byte_idx == adp_hi_idx) begin
          if (is_auto_inc_cmd)
            tx_byte_comb = rx_data - {7'd0, adp_dec_borrow};
          ...
          case (cmd_reg)
            CMD_APRD, CMD_FPRD, CMD_BRD:
              wkc_calc_comb = read_success ? 2'd1 : 2'd0;
            CMD_APWR, CMD_FPWR, CMD_BWR:
              wkc_calc_comb = write_success ? 2'd1 : 2'd0;
