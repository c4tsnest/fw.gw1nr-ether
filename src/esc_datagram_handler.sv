module esc_datagram_handler (
  ...
  always_comb begin
    ...
    if (rx_valid) begin
      tx_byte_comb = rx_data;

      if (is_ethercat) begin
        if (byte_idx == cmd_idx) begin
          // pass-through
        end else if (byte_idx == adp_lo_idx) begin
          if (is_auto_inc_cmd) tx_byte_comb = rx_data - 8'h01;
        end else if (byte_idx == adp_hi_idx) begin
          if (is_auto_inc_cmd) tx_byte_comb = rx_data - {7'd0, adp_dec_borrow};
          ...
          // pass-through
        end else if (addr_match && (byte_idx >= data_start_idx) &&
                     (byte_idx < (data_start_idx + {5'd0, dlen_reg}))) begin
          if (do_read) tx_byte_comb = reg_if.rd_data;
          if (do_write) begin
            reg_if.wr_en   = 1'b1;
            reg_if.wr_addr = byte_addr;
            reg_if.wr_data = rx_data;
          end
        end else if (addr_match && (byte_idx == wkc_start_idx)) begin
          case (cmd_reg)
            CMD_APRD, CMD_FPRD, CMD_BRD: wkc_calc_comb = read_success ? 2'd1 : 2'd0;
            CMD_APWR, CMD_FPWR, CMD_BWR: wkc_calc_comb = write_success ? 2'd1 : 2'd0;
            CMD_APRW, CMD_FPRW, CMD_BRW: begin
              wkc_calc_comb = 2'd0;
              if (read_success) wkc_calc_comb = wkc_calc_comb + 2'd1;
              if (write_success) wkc_calc_comb = wkc_calc_comb + 2'd2;
            end
            default: wkc_calc_comb = 2'd0;
          endcase
          tx_byte_comb = rx_data + {6'd0, wkc_calc_comb};
        end else if (addr_match && (byte_idx == (wkc_start_idx + 16'd1))) begin
          tx_byte_comb = rx_data + {7'd0, wkc_carry};
        end
      end
  ...
  assign tx_out.data = tx_byte_comb;
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      if (rx_valid) begin
        automatic byte_t rx_byte = rx_data;

        if (is_ethercat) begin
          if (byte_idx == cmd_idx) begin
            cmd_reg <= rx_byte;
            case (rx_byte)
              CMD_APRD, CMD_APWR, CMD_APRW: is_auto_inc_cmd <= 1'b1;
              CMD_FPRD, CMD_FPWR, CMD_FPRW, CMD_BRD, CMD_BWR, CMD_BRW: is_auto_inc_cmd <= 1'b0;
              default: is_auto_inc_cmd <= 1'b0;
            endcase
          end else if (byte_idx == adp_lo_idx) begin
            adp_reg[7:0] <= rx_byte;
          ...
          if (addr_match && (byte_idx == wkc_start_idx)) begin
            logic [1:0] wkc_calc;
            case (cmd_reg)
              CMD_APRD, CMD_FPRD, CMD_BRD: wkc_calc = read_success ? 2'd1 : 2'd0;
              CMD_APWR, CMD_FPWR, CMD_BWR: wkc_calc = write_success ? 2'd1 : 2'd0;
              CMD_APRW, CMD_FPRW, CMD_BRW: begin
                wkc_calc = 2'd0;
                if (read_success) wkc_calc = wkc_calc + 2'd1;
                if (write_success) wkc_calc = wkc_calc + 2'd2;
              end
              default: wkc_calc = 2'd0;
            endcase
            wkc_inc_value <= wkc_calc;
            wkc_carry <= ({1'b0, rx_byte} + {7'd0, wkc_calc} > 9'h0ff);
            debug_wkc_value[7:0] <= tx_out.data;
          end else if (addr_match && (byte_idx == (wkc_start_idx + 16'd1))) begin
            debug_wkc_value[15:8] <= tx_out.data;
          end
        end
      end
    end
