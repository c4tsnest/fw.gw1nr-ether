module esc_regfile #(
  ...
  always_comb begin
    reg_if.write_hit_cmb = reg_can_write_addr(reg_if.wr_addr) &&
                           !(reg_eep_cfg[8] == 1'b1 &&
                             ((reg_if.wr_addr >= REG_EEP_STAT &&
                               reg_if.wr_addr < (REG_EEP_STAT + 2)) ||
                              (reg_if.wr_addr >= REG_EEP_ADDR &&
                               reg_if.wr_addr < (REG_EEP_ADDR + 4)) ||
                              (reg_if.wr_addr >= REG_EEP_DATA &&
                               reg_if.wr_addr < (REG_EEP_DATA + 4))));
  end
      ...
      if (reg_if.wr_en) begin
        ...
        if (reg_can_write_addr(reg_if.wr_addr)) begin
          if (addr_in_window(reg_if.wr_addr, '0, CORE_REG_BYTES)) begin
            ...
            reg_irq_mask[(reg_if.wr_addr - REG_IRQ_MASK) * 8 +: 8]
                <= reg_if.wr_data;
            write_hit = 1'b1;
            ...
            if (reg_eep_cfg[8] == 1'b0) begin
              reg_eep_addr[(reg_if.wr_addr - REG_EEP_ADDR) * 8 +: 8]
                  <= reg_if.wr_data;
              write_hit = 1'b1;
            end
            ...
            if (reg_eep_cfg[8] == 1'b0) begin
              reg_eep_data[(reg_if.wr_addr - REG_EEP_DATA) * 8 +: 8]
                  <= reg_if.wr_data;
              write_hit = 1'b1;
            end
