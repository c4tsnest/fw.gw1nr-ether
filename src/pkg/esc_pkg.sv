package esc_pkg;
  ...
  function automatic logic addr_in_window(input addr_t addr, input addr_t base,
                                          input int unsigned size);
    addr_in_window = (addr >= base) && (addr < (base + addr_t'(size)));
  endfunction

  function automatic reg_access_t esc_reg_access(
      input addr_t addr, input int unsigned gpio_out_bytes, input int unsigned gpio_in_bytes);
    ...
    if ((addr >= REG_GPIO_OUT) && (addr < (REG_GPIO_OUT + addr_t'(gpio_out_bytes))))
      esc_reg_access = REG_RW;
    ...
    if ((addr >= REG_GPIO_IN) && (addr < (REG_GPIO_IN + addr_t'(gpio_in_bytes))))
      esc_reg_access = REG_R;
